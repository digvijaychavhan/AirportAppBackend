...
Parses spoken transcripts & text prompts into structured JSON UI actions & stop lists
using Groq LLM (llama-3.3-70b-versatile) with fallback regex intent parsing.
"""
...
def fallback_intent_parser(transcript: str, kiosk_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Rule-based and regex fallback parser when Groq API key is unavailable or request fails.
    """
    ...
    # Category 2: Directory / Category Page Routing
    ...
    # Category 3: Map Direction & Multi-Stop Extraction
    poi_keywords = [
...
async def parse_ai_intent(transcript: str, kiosk_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Invokes Groq LLM model (llama-3.3-70b-versatile) to convert passenger prompt into structured intent JSON.
    Uses fallback regex parser if Groq API is unavailable or key is missing.
    """
    ...
    if not groq_api_key:
        logger.info("GROQ_API_KEY not set. Using fallback intent parser.")
    ...
    try:
        ...
        logger.warning(f"Groq API call fallback due to error: {e}")
        return fallback_intent_parser(transcript, kiosk_context)


# Alias function for routes/ai.py router
